...
├── docs/            # 仕様書(SPEC.md) / ER図 / 開発ガイド / 各担当手順書(PDF)
└── README.md
```
...
ブラウザで http://localhost:5173 を開く。環境変数は [`backend/.env.example`](backend/.env.example) を参照。
...
はじめて参加する場合は以下の順で読むことを推奨します。

1. [`docs/concepts.md`](docs/concepts.md) — Webアプリの基本概念
2. [`docs/setup.md`](docs/setup.md) — 環境構築完全ガイド
3. [`docs/git-guide.md`](docs/git-guide.md) — git の使い方
4. [`docs/development-order.md`](docs/development-order.md) — 開発の進め方
5. `docs/pdf/<あなたの名前>.pdf` — 担当ごとの手順書

詳細仕様は [`docs/SPEC.md`](docs/SPEC.md)、バックエンド構成は [`docs/backend-flow.md`](docs/backend-flow.md) を参照。
...
Oracle Cloud Always Free VM 1台で稼働。コード更新後の再デプロイは:

```bash
./deploy/redeploy.sh   # 最新をrsync → VMで再ビルド → サービス再起動
```

1. 初期セットアップ: [`deploy/setup.sh`](deploy/setup.sh)
...
4. 環境変数テンプレート: [`deploy/labsoldier.env.example`](deploy/labsoldier.env.example)

**永続データ（DB・env）は同期対象外のディレクトリに置き**、コード再デプロイ（`rsync --delete`）で消えないようにしています。
