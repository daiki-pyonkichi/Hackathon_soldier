...
> kuremoto はコーディング無し（デザイン専任）。3人のコーダーで各自1スライス担当。
...
| --- | --- | --- |
| **naganawa** | PM + **ログイン機能** | 認証フロー全体（FE+BE）/ `/api/auth/*` / `/api/me` / `Login.tsx` / トークン保管 / ログアウト / PR レビュー / IP 実測 / デプロイ |
...
| **kuremoto** | **デザイン** | キャラ素材（Canva）/ 成長/虐待の段階デザイン / 見取り図モック / アイコン素材 / デモ動画 |
...
└─────────────────────┘   └─────────────────────┘
              ↑
              └ kuremoto: 素材 (Canva) → frontend/public/
```
