...
- リポジトリ: `daiki-pyonkichi/Hackathon_soldier`（private）
...
| カラム | 型 | 説明 |
| --- | --- | --- |
| id | TEXT PK | ユーザーID（例 `u-naganawa` / UUID） |
...
| 配置先 | `/opt/labsoldier`（rsyncで転送。privateリポジトリのためVMでcloneはしない） |
| プロセス | systemd `labsoldier.service`（`node dist/index.js`, port 3001, 自動再起動） |
| 環境変数 | `/opt/labsoldier/labsoldier.env`（JWT_SECRET / LAB_ALLOWED_IPS / DB_PATH 等） |
| HTTPS | Caddy が `161.33.21.136.sslip.io` でLet's Encrypt証明書を自動取得 |
...
DB（`backend/data`）はrsync除外なので保持される。
