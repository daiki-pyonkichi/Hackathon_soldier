...
```
クライアント
  └─ POST /api/auth/login { "name": "tsutsumi" }
        ↓
...
[store.ts:65]      store.listUsers()         ← users 全件取得
        ↓
                   配列から name=="tsutsumi" を find
        ↓
                   見つかれば { user, token: "mock-u-tsutsumi" } を返す
                   見つからなければ 404
        ↓
クライアント
```
...
```
クライアント (Authorization: Bearer mock-u-tsutsumi)
...
TOKEN=$(curl -s -X POST http://localhost:3001/api/auth/login \
  -H "Content-Type: application/json" -d '{"name":"tsutsumi"}' \
  | sed 's/.*"token":"\([^"]*\)".*/\1/')
